    ...
    private readonly struct LuaStackChecker : IDisposable {
        ...
        public void Dispose() {
            ...
            if (newSize != size + popExtraValues) {
                var message = $"Lua interop stack did not remain balanced (was {size}, expected {size + popExtraValues}, now {newSize}).";
                Yafc.App.Services.AppLog.Write($"[StackChecker WARN] {message}");

                // Force stack back to expected state to keep subsequent operations valid.
                try { lua_settop(ctx.L, size + popExtraValues); } catch { }

                // Only throw when there is NO pending exception, so we don't mask the real Lua error.
